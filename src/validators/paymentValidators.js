...
const webhookSchema = Joi.object({
  ...
  status: Joi.string().valid(...VALID_PAYMENT_STATUSES).required(),
...
const paymentValidators = {
  ...
  refundSchema: Joi.object({
    ...
    amount: Joi.number().positive().max(Joi.ref('/originalAmount')),
    notes: Joi.string(),
    requested_by: Joi.number().required(),
  }),
  tipSchema: Joi.object({
    amount: Joi.number().positive().required(),
    allocation: Joi.object({
      driver_percentage: Joi.number().min(0).max(100),
      staff_percentage: Joi.number().min(0).max(100),
    }).optional(),
  }),
};
