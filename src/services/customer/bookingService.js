...
const { Booking, Table, Customer, Merchant, Notification, BookingTimeSlot, Feedback } = require('@models');
...
const bookingService = {
  ...
  async checkInBooking({ bookingId, merchantId }) {
    ...
    // Request feedback after check-in
    await bookingService.requestFeedback(bookingId, booking.customer_id);

    logApiEvent('Customer checked in', { bookingId, digitalMenuUrl });
    return booking;
  },

  async requestFeedback(bookingId, customerId) {
    const booking = await Booking.findByPk(bookingId);
    ...
    await Notification.create({
      ...
      message: `How was your experience with booking #${booking.reference}? Leave a review!`,
      priority: 'LOW',
    });

    logger.info('Feedback requested for booking', { bookingId, customerId });
  },

  async submitFeedback(bookingId, customerId, staffId, rating, comment) {
    const booking = await Booking.findByPk(bookingId);
    if (!booking) throw new AppError('Booking not found', 404, 'BOOKING_NOT_FOUND');
    if (booking.customer_id !== customerId) throw new AppError('Unauthorized', 403, 'UNAUTHORIZED');
    if (booking.status !== 'seated') throw new AppError('Booking not seated', 400, 'BOOKING_NOT_SEATED');
    ...
    logger.info('Feedback submitted for booking', { bookingId, customerId, staffId, rating });
    return feedback;
  },
